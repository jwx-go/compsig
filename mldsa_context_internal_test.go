...
// signing through jwsbb -> jwx-go/mldsa, which hard-codes ctx=nil; the
...
func TestMLDSAContextDomainSeparation(t *testing.T) {
	for _, info := range compSigAlgs {
		t.Run(info.name, func(t *testing.T) {
			...
			require.Error(t, err, "mldsa.Verify with empty ctx must fail (would mean Sign dropped ctx=Label)")
		})
	}
}
